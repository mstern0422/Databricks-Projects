...
grades.filter(expr('term_id == "2021A"')).join(other = course, how = 'outer').groupBy('dept').agg(expr('COUNT(sid) AS n_students'), expr('ROUND(COUNT(sid) / {}, 4) AS prop'.format(sp21_enr))).sort('prop', ascending = False).show()
